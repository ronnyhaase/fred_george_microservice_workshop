...
class RentalSolutionCollector
  ...
  def initialize(host_ip, port)
    ...
    @service_name = 'supply_rental_solution_collector_ruby_' + SecureRandom.uuid
  end
  ...
  def packet rapids_connection, packet, warnings
    #puts packet.inspect
    @solutions[packet.id] = better(@solutions[packet.id], packet)
    puts "The winner is: #{@solutions[packet.id]}"
  end

  private

    def better(champion, challenger)
      return challenger unless champion
      @solutions[challenger.id] = survivor(challenger, champion)
    end

    def survivor(challenger, champion)
      power(challenger) > power(champion) ? challenger : champion
    end

    def power(candidate)
      candidate.solution['likelihood'] * candidate.solution['value']
    end
end

RentalSolutionCollector.new(ARGV.shift, ARGV.shift).start
